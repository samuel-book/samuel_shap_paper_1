...
def waterfall(shap_values, max_display=10, show=True, y_reverse=False):
    ...
    if y_reverse:
        ...
        ax2.set_xticks([base_values + values.sum(), base_values + values.sum() + 1e-8]) # provide two locations as using two tick labels (one for "f(x)" and one for "= value"
        # The 1e-8 is so matplotlib 3.3 doesn't try and collapse the ticks
        ax2.set_xticklabels(["$f(x)$", "$ = "+format_value(fx, "%0.03f")+"$"], fontsize=12, ha="left")
        ...
        # The 1e-8 is so matplotlib 3.3 doesn't try and collapse the ticks
        ax3.set_xticks([base_values, base_values+1e-8])  # provide two locations as using two tick labels (one for "f(x)" and one for "= value"
        ax3.set_xticklabels(["\n$E[f(X)]$", "\n$ = "+format_value(base_values, "%0.03f")+"$"], fontsize=12, ha="left") # ha is horizontal alignment
        ...
        ) + matplotlib.transforms.ScaledTranslation(22/72., 0, fig.dpi_scale_trans))
        ...
        tick_labels[0].set_transform(tick_labels[0].get_transform(
        ) + matplotlib.transforms.ScaledTranslation(-10/72., -15/72., fig.dpi_scale_trans))
        tick_labels[1].set_transform(tick_labels[1].get_transform(
        ) + matplotlib.transforms.ScaledTranslation(12/72., -15/72., fig.dpi_scale_trans))
        tick_labels[1].set_color("#999999")
        ...
        ax2.set_xlim(xmin, xmax)
        # The 1e-8 is so matplotlib 3.3 doesn't try and collapse the ticks
        ax2.set_xticks([base_values, base_values+1e-8])  # provide two locations as using two tick labels (one for "f(x)" and one for "= value"
        ax2.set_xticklabels(["\n$E[f(X)]$", "\n$ = "+format_value(base_values, "%0.03f")+"$"], fontsize=12, ha="left") # ha is horizontal alignment
        ax2.spines['right'].set_visible(False)
        ax2.spines['top'].set_visible(False)
        ax2.spines['left'].set_visible(False)
        ...
        ax3.set_xlim(xmin, xmax)
        # The 1e-8 is so matplotlib 3.3 doesn't try and collapse the ticks
        ax3.set_xticks([base_values + values.sum(), base_values + values.sum() + 1e-8]) # provide two locations as using two tick labels (one for "f(x)" and one for "= value"
